<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold text-gray-800">Edit Deduction</h2>
    </x-slot>

    <div class="py-6 max-w-3xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white shadow rounded-lg p-6">
            <form action="{{ route('deductions.update', $deduction->id) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="unit_id" class="border px-3 py-2 w-full rounded">
                        @foreach($units as $unit)
                            <option value="{{ $unit->id }}" {{ $deduction->unit_id == $unit->id ? 'selected' : '' }}>
                                {{ $unit->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="department_id" class="border px-3 py-2 w-full rounded">
                        @foreach($departments as $dept)
                            <option value="{{ $dept->id }}" {{ $deduction->department_id == $dept->id ? 'selected' : '' }}>
                                {{ $dept->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ $deduction->name }}" 
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="amount" value="{{ $deduction->amount }}" 
                           class="border px-3 py-2 w-full rounded">
                    @error('amount')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
