<x-app-layout>
    ...
    <!-- ADD UNIT + MAP + SEARCH -->
    <div x-data="{ addUnitOpen: false, mapOpen: false, lat: '', lng: '' }">
        <div class="py-6">
            <button @click="addUnitOpen = true" class="bg-blue-600 text-white px-4 py-2 rounded"> Add New Unit </button>

            <!-- UNIT MODAL -->
            <div x-show="addUnitOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                <div @click.away="addUnitOpen = false" class="bg-white rounded-lg shadow-lg w-full max-w-sm p-6">
                    ...
                    <form method="POST" action="{{ route('admin.units.store') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="radius" class="w-full border rounded px-3 py-2" placeholder="e.g. 100" required>
                        </div>
                        <!-- SET LOCATION -->
                        <button type="button" @click="mapOpen = true" class="bg-green-600 text-white px-3 py-1 rounded"> Set Location </button>
                        <input type="hidden" name="latitude" x-model="lat">
                        <input type="hidden" name="longitude" x-model="lng">
                        <div class="flex justify-end mt-4 space-x-2">
                            <button type="button" @click="addUnitOpen = false" class="bg-gray-300 px-4 py-2 rounded"> Cancel </button>
            ...
            <div x-show="mapOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div class="bg-white p-4 rounded w-full max-w-2xl">
                    <h2 class="text-lg font-bold mb-2">Select Unit Location</h2>
                    <!-- SEARCH INPUT -->
                    <input type="text" id="locationSearch" placeholder="Search location..." class="w-full border p-2 rounded mb-2">
                    <!-- SUGGESTIONS -->
                    <div id="suggestions" class="border rounded max-h-40 overflow-y-auto text-sm mb-2"></div>
                    <!-- MAP -->
                    <div id="map" style="height: 400px;"></div>
                    <div class="flex justify-end mt-3">
                        <button @click="mapOpen = false" class="bg-gray-500 text-white px-3 py-1 rounded"> Close </button>
                    </div>
                </div>
            </div>
        </div>
